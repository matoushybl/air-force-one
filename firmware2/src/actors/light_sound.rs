use drogue_device::drivers::led::neopixel::rgbw::{Rgbw8, GREEN, RED};
...
pub struct LightSoundReactor {
    buzzer: Address<Bzzz>,
    neopixel: Address<Rgbw8>,
    bzzz_time: Option<Instant>,
}

impl LightSoundReactor {
    pub fn new(buzzer: Address<Bzzz>, neopixel: Address<Rgbw8>) -> Self {
        Self {
            buzzer,
            neopixel,
            bzzz_time: None,
    ...
    async fn process_air_quality(&mut self, air_quality: &AirQuality) {
        ...
        // self.neopixel.notify(rgb).await;
    }
}
        ...
        loop {
            if let Some(bzzz_time) = self.bzzz_time {
                match select(inbox.next(), Timer::at(bzzz_time)).await {
                    embassy::util::Either::First(air_quality) => {
                        self.process_air_quality(&air_quality).await
                    }
                    embassy::util::Either::Second(_bzzz) => {
